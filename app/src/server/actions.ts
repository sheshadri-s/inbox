...
import { getEmailAnalytics } from './sendmail/sesUtils';
...
export const emaildata = async (data : any) => {
 const value = data.data
 await getEmailAnalytics({value})
}
...
export const hellno = ({ to, from, subject, body, tag } : any) => {
  sendEmail({to, from, subject, body, tag})
}
...
export const createCampaign: CreateCampaign<{
...
}, Campaign> = async ({ name, to, from, subject, body, tag, schedule }, context) => {
  ...
  const matchingEmails = await context.entities.Task.findMany({
    where: { Tag: to },
  });

  // Extract recipient emails into an array
  const emailArray = matchingEmails.map(task => task.email);



  // Generate unique IDs for each email
  const emailData = emailArray.map((email : any) => ({
  ...
  const emailIDs = emailData.map((email : any) => email.id);

  // Send emails with unique IDs
  sendEmail({ to, from, subject, body, tag, schedule, emailIDs });
